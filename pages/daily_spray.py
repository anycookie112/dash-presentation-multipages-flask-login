...
def fetch_data_spray(date_start='2024-12-11', date_end='2025-03-21'):
    query = f"""
        SELECT spray_batch_info.spray_batch_id , spray_batch_info.part_code, spray_batch_info.date_sprayed,
               history_spray.movement_date, history_spray.movement_reason, history_spray.amount_inspect, history_spray.amount_reject,
               history_spray.checker_name, spray_defect_list.*
        ...
        WHERE history_spray.movement_date >= '{date_start}' AND history_spray.movement_date < '{date_end}'
        ORDER BY history_spray.movement_date
    """
    with db_connection_str.connect() as connection:
        df = pd.read_sql(query, connection)

        df_grouped = df.copy()
        df_grouped = df_grouped.drop(columns=['checker_name'])
        df_grouped = df_grouped.groupby(['spray_batch_id'])
    
    return df, df_grouped


def filter_spray_data(spray_batch_id = None, date_start='2024-12-11', date_end='2025-03-21'):
        if not spray_batch_id:
            print("spray_batch_id cannot be empty or None")
        query = f"""
        SELECT spray_batch_info.spray_batch_id , spray_batch_info.part_code, spray_batch_info.date_sprayed,
               history_spray.movement_date, history_spray.movement_reason, history_spray.amount_inspect, history_spray.amount_reject,
               history_spray.checker_name, spray_defect_list.*
        ...
        INNER JOIN spray_defect_list 
            ON spray_defect_list.spray_inspection_id = history_spray.spray_inspection_id
        WHERE history_spray.movement_date >= '{date_start}' AND history_spray.movement_date < '{date_end}' AND spray_batch_info.spray_batch_id = '{spray_batch_id}'
        ORDER BY history_spray.movement_date
    """
...
def get_part_codes():
    query = """
        SELECT DISTINCT part_code FROM spray_batch_info
    """
    with db_connection_str.connect() as connection:
        df = pd.read_sql(query, connection)
...
def all_data(spray_batch_id=None):
    if spray_batch_id is None:
        spray_batch_id = 0
    ...
    with db_connection_str.connect() as connection:
        df = pd.read_sql(query, connection)
        df_ordered = df.sort_values(by='movement_date', ascending=True)
    return df_ordered



def overall_data(spray_batch_id=None):
        if spray_batch_id is None:
            spray_batch_id = 0
        query = f"""
            SELECT spray_batch_info.spray_batch_id , spray_batch_info.part_code, spray_batch_info.date_sprayed,
        ...
        with db_connection_str.connect() as connection:
            ...
            df_grouped = df.copy()
            df_grouped.drop(columns=['spray_inspection_id'], inplace=True)

            group_cols = ['spray_batch_id', 'movement_reason', 'checker_name']
            group_cols2 = ['spray_batch_id', 'movement_reason']
...
def fetch_data_spray(date_start=None, date_end=None):
    ...
    with db_connection_str.connect() as connection:
        ...
        df_grouped = df_grouped.groupby(['spray_batch_id'])
        # group df just groups all related to 1 spray_batch_id together
...
def filter_spray_data(spray_batch_id=None, date_start=None, date_end=None):
    if not spray_batch_id:
        spray_batch_id = 50  # Default value if not provided
    if not date_start or not date_end:
        today = datetime.date.today()
        yesterday = today - datetime.timedelta(days=1)
        if not date_start:
...
grouped = fetch_data_spray()
grouped_df = grouped.first().reset_index() 
df = filter_spray_data()
df_chart, df_table = overall_data(50)
...
def layout():
    ...
    html.H1('Daily Spray Inspection Daily Report'),
    html.Div(
        [
            dcc.DatePickerRange(
...
def update_table_overall(start_date, end_date, part_code):
    ...
    df_grouped = fetch_data_spray(start_date, end_date)
...
def get_lot_data(rowdata, date_start, date_end):
    ...
    if 'spray_batch_id' in selected_row:
        spray_batch_id = selected_row['spray_batch_id']
        df = filter_spray_data(spray_batch_id=spray_batch_id, date_start = date_start, date_end = date_end)
        ...
        chart_data, table_data = overall_data(spray_batch_id)
        ...
        df_all = all_data(spray_batch_id)
